...
class Item {
    constructor(clickState, id, number, livedOnce) {
...
function handleClickedElement(id) {
...
function updateGridPixelSize() {
    ...
    let cgolPitchTemp = document.getElementsByTagName("cgol-pitch")[0];
    let firstElement = cgolPitchTemp.shadowElement.querySelector("#container");
...
function resizeGrid() {
    let newHeightTemp = document.getElementById("height");
    let newWidthTemp = document.getElementById("width");
...
function changeGridHeightAndWith(newHeight, newWidth) {
    clearGame();
    rows = newHeight;
    cols = newWidth;
    let cgolPitchTemp = document.getElementsByTagName("cgol-pitch")[0];
...
function setTime() {
    let newTime = document.getElementById("timer");
